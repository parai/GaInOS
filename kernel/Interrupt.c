...
/* |-------------------+----------------------------------------------| */
#include "Kernel.h"
/* Machine State Register of Cpu For EnableAllInterrupts()/DisableAllInterrupts()*/ 
static OsCpuSrType  _Sr;
/* Machine State Register of Cpu For SuspendAllInterrupts()/ResumeAllInterrupts()*/ 
static OsCpuSrType  _CpuSr;                                         
static uint8_t      _EnDeIntNest  =0x00u; /* Interrupt call Enable and Disable Nest*/

/* Machine State Register of Cpu For SuspendOSInterrupts()/ResumeOSInterrupts()*/
static OsCpuSrType  _OSCpuSr;    
static uint8_t      _OSEnDeIntNest=0x00u; /* OS Interrupt call Enable and Disable Nest*/
...
{
    vPortSaveMsrAndDisableInterrupt(_Sr);
}
...
void ResumeAllInterrupts ( void )
{
    ...
    if(0x00u == _EnDeIntNest)
    {
        vPortRestoreMsr(_CpuSr);
    }
}
...
void SuspendAllInterrupts( void )
{
    if(0 == _EnDeIntNest)
    {
        vPortSaveMsrAndDisableInterrupt(_CpuSr);
        _EnDeIntNest++;
    }
    else if(_EnDeIntNest < 0xFF){
        _EnDeIntNest++;
    }
}
...
void SuspendOSInterrupts( void )
{
    if(0 == _OSEnDeIntNest)
    {
        vPortSaveMsrAndDisableInterrupt(_OSCpuSr);
    }

    if(_OSEnDeIntNest < 0xFF){
        _OSEnDeIntNest++;
    }
}
...
void ResumeOSInterrupts( void )
{
    ...
    if(0x00u == _OSEnDeIntNest)
    {
        vPortRestoreMsr(_OSCpuSr);
    }

}

static uint8_t OSCritialNest = 0x00u;
static OsCpuSrType OSCriticalSR = 0x00u;

void OS_ENTER_CRITICAL(void)
{
    if(OSCritialNest == 0)
    {
        vPortSaveMsrAndDisableInterrupt(OSCriticalSR);
    }
    if(OSCritialNest < 0xFF)
    {
        OSCritialNest++;
    }
}
