...
#include "Kernel.h"
#if((cfgOS_ALARM_NUM >0) || (cfgOS_SCHEDULE_TABLE_NUM>0))
/* This Api is shared by Alarm and Schedule Table,Calculate the next processing time */
                                  ...
                                  TickRefType Value,TickRefType ElapsedValue)
{
    ...
    if(*Value > tableGetCntMaxAllowed(CounterID))
    {
        xRet = E_OS_VALUE;
        goto Error_Exit;
    }
#endif
    if(tableGetCntCurValue(CounterID) > *Value)
    {
        *ElapsedValue = tableGetCntCurValue(CounterID) - *Value;
    }
    else
    {
...
#endif  /* #if((cfgOS_ALARM_NUM >0) || (cfgOS_SCHEDULE_TABLE_NUM>0)) */
